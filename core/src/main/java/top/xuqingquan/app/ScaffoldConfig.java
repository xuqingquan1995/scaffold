    ...
    public static Cache.Factory getCacheFactory() {
        if (cacheFactory == null) {
            cacheFactory = type -> {
                ...
                return switch (type.getCacheTypeId()) {
                    //Activity、Fragment 以及 Extras 使用 IntelligentCache (具有 LruCache 和 可永久存储数据的 Map)
                    case CacheType.EXTRAS_TYPE_ID, CacheType.ACTIVITY_CACHE_TYPE_ID, CacheType.FRAGMENT_CACHE_TYPE_ID ->
                            new IntelligentCache(type.calculateCacheSize(application));
                    //其余使用 LruCache (当达到最大容量时可根据 LRU 算法抛弃不合规数据)
                    default -> new LruCache(type.calculateCacheSize(application));
                };
            };
        }
        return cacheFactory;
