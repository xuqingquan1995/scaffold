...
object FastBlur {
    ...
    fun blur(context: Context, bkg: Bitmap, view: View) {
        ...
        val scaleFactor = 8f
        //放大到整个view的大小
        val bitmap = DrawableProvider.getReSizeBitmap(bkg, view.measuredWidth.toFloat(), view.measuredHeight.toFloat())
    ...
    fun blurBitmap(bkg: Bitmap, width: Int, height: Int): Bitmap? {
        ...
        val scaleFactor = 8f
        //放大到整个view的大小
        val bitmap = DrawableProvider.getReSizeBitmap(bkg, width.toFloat(), height.toFloat())
        var overlay: Bitmap? = Bitmap.createBitmap(
