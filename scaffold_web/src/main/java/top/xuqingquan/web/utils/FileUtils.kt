...
fun getUriFromFile(context: Context, file: File): Uri {
    return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
        ScaffoldWebFileProvider.getUriForFile(context, context.packageName + ".ScaffoldFileProvider", file)
    } else {
        Uri.fromFile(file)
    }
}
...
fun clearCacheFolder(dir: File?, numDays: Int): Int {
    var deletedFiles = 0
    if (dir != null) {
        LogUtils.i("dir:" + dir.absolutePath)
    }
        ...
        try {
            for (child in dir.listFiles() ?: arrayOf<File>()) {
                ...
                if (child.lastModified() < Date().time - numDays * DateUtils.DAY_IN_MILLIS) {
                    LogUtils.i("file name:" + child.name)
                    if (child.delete()) {
                        deletedFiles++
                    }
                }
            ...
            LogUtils.e(e, "Failed to clean the cache, result %s", e.message)
        }
